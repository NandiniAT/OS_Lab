    ...
    for (int i = 0; i < n_files; i++) {
        for (int j = 0; j < n_blocks; j++) {
            if (blocks[j].is_free && blocks[j].block_size >= files[i].file_size) {
                blocks[j].is_free = 0;
                printf("%d\t\t%d\t\t%d\t\t%d\t\t%d\n", files[i].file_no, files[i].file_size, blocks[j].block_no, blocks[j].block_size, blocks[j].block_size - files[i].file_size);
                break;
            }
        }
    }
...
void bestFit(struct Block blocks[], int n_blocks, struct File files[], int n_files) {
    ...
    for (int i = 0; i < n_files; i++) {
        ...
        if (best_fit_block!= -1) {
            ...
            printf("%d\t\t%d\t\t%d\t\t%d\t\t%d\n", files[i].file_no, files[i].file_size, blocks[best_fit_block].block_no, blocks[best_fit_block].block_size, min_fragment);
        }
    }
}
int main() {
